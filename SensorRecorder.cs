...
public class SensorRecorder : MonoBehaviour
{
    ...
    void SampleHands()
    {
        SampleHand(HandType.HandLeft, "left");
        SampleHand(HandType.HandRight, "right");
    }

    void SampleHand(HandType ht, string label)
    {
        HandJointsLocations jl = new HandJointsLocations();
        try { if (!PXR_HandTracking.GetJointLocations(ht, ref jl)) return; } catch { SampleHandXR(label); return; }
        if (jl.jointLocations == null) return;
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            var p = j.pose.Position.ToVector3(); var q = j.pose.Orientation.ToQuat();
    ...
    void SampleHandXR(string label)
    {
        var subs = new List<XRHandSubsystem>(); SubsystemManager.GetSubsystems(subs);
        foreach (var sub in subs)
        {
            if (!sub.running) continue;
            XRHand h = label == "left" ? sub.leftHand : sub.rightHand;
            if (!h.isTracked) continue;
            for (int i = 0; i < XRHandJointID.EndMarker.ToIndex(); i++)
            {
                var jid = XRHandJointIDUtility.FromIndex(i);
                var j = h.GetJoint(jid);
                if (!j.TryGetPose(out Pose pose)) continue;
                j.TryGetRadius(out float rad);
                string jn = i < JN.Length ? JN[i] : jid.ToString();
                _handW.WriteLine($"{_elapsed:F6},{FrameIndex},{label},{i},{jn},{pose.position.x:F6},{pose.position.y:F6},{pose.position.z:F6},{pose.rotation.x:F6},{pose.rotation.y:F6},{pose.rotation.z:F6},{pose.rotation.w:F6},{rad:F4}");
            }
            break;
        }
    }
